...
JOIN pedidos ON pp.pedido_id  = pedidos.id;
-- 2)
SELECT DISTINCT  pp.pedido_id
FROM 
	pedidos p 
JOIN produtos_pedidos AS pp ON pp.produto_id =6 ;

-- 3)
SELECT DISTINCT  c.nome AS "gostam_de_fritas"
...
-- 4)
SELECT sum(Round(CAST(p2.preco AS numeric),2))
FROM  
